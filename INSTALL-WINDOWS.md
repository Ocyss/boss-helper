...
V2 只在 BOSS 直聘页面工作。“自动投递（含招呼语）”默认关闭；关闭时不会真实投递，流程会停在“待人工”。回复监控默认关闭，开启后只发 Chrome 通知并保存待人工确认的草稿队列。V2 不实现自动回复、自动发简历或自动交换联系方式。
...
4. 开关只覆盖岗位投递和首次文本招呼语，不会自动回复后续消息、发简历、交换联系方式或绕过验证码/风控。构建和冒烟测试不会开启该开关，也不会使用真实账号发送。
...
- 关闭自动投递时，招呼语仅作为岗位详情中的草稿展示；需要人工发送时仍由用户自己复制或填入并点击发送。开启自动投递后仅允许当前岗位的文本招呼语走既有 BOSS 聊天通道。
